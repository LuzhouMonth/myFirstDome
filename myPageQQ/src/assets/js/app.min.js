$(document).ready(function(){$('#mainNavTab a').on('click',function(e){e.preventDefault()
$(this).tab('show')})
...
$('[data-chat-info-toggle]').on('click',function(e){e.preventDefault()
$(".chat-info").addClass("chat-info-visible")})
$('[data-chat-info-close]').on('click',function(e){e.preventDefault()
$(".chat-info").removeClass("chat-info-visible")})
$('.contacts-list .contacts-link').on('click',function(){$(".main").addClass("main-visible")})
$('.contacts-list .media-link').on('click',function(){$(".main").addClass("main-visible")})
$('[data-profile-edit]').on('click',function(){$(".main").addClass("main-visible")})
$('[data-close]').on('click',function(e){e.preventDefault()
$(".main").removeClass("main-visible")})
$('.chat-content').magnificPopup({delegate:'a.popup-media',type:'image',gallery:{enabled:!0,navigateByImgClick:!0,preload:[0,1]}});$('[data-chat-filter]').on('click',function(){let selectedOption=$(this).data('select');$('[data-chat-filter-list]').text($(this).text());if(selectedOption==='all-chats'){$('[data-chat-list]').find('li').each(function(){$(this).show()})}else{$('[data-chat-list]').find('li').each(function(){$(this).hide()});$('[data-chat-list] li.'+selectedOption).show()}});$('[data-call-filter]').on('click',function(){let selectedOption=$(this).data('select');$('[data-call-filter-list]').text($(this).text());if(selectedOption==='all-calls'){$('[data-call-list]').find('li').each(function(){$(this).show()})}else{$('[data-call-list]').find('li').each(function(){$(this).hide()});$('[data-call-list] li.'+selectedOption).show()}});$('#createGroup').modalSteps({btnNextHtml:"Next",btnLastStepHtml:"Finish",disableNextButton:!1,completeCallback:function(){},callbacks:{},getTitleAndStep:function(){}});$(document).on('change','.custom-file-input',function(event){$(this).next('.custom-file-label').html(event.target.files[0].name)})
